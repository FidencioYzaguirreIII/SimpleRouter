...
struct sr_rt* longestPrefixMatch(struct sr_instance *sr,uint32_t ipDst){
    ...
    while(routeTable){
 	uint32_t result = routeTable->mask.s_addr & routingTarget.s_addr;
	if(result == routeTable->dest.s_addr){
		if(ntohl(routeTable->mask.s_addr) >= ntohl(largestMask.s_addr)){
			match = routeTable;
			largestMask = routeTable->mask;
		}
	}
