  ...
  if (difftime(now, req->sent) >= 1.0)
  {
    if (req->times_sent >= 5)
    {
	  ...
	  while (pkt != NULL) 
	  {
		...
		struct sr_rt* tableMatch  = longestPrefixMatch(sr,ipPkt->ip_src);
		if (!tableMatch){
			return;
		}
    ...
    if (req != NULL)
    {
      ...
      /* TODO: send all packets on the req->packets linked list            */
		while (req->packets != NULL)
		{
		        ...
		        memcpy(((sr_ethernet_hdr_t*)pkt->buf)->ether_dhost, arphdr->ar_sha, ETHER_ADDR_LEN);
...
void sr_handlepacket_ip(struct sr_instance *sr, uint8_t *pkt,
    unsigned int len, struct sr_if *src_iface){
    ...
    if(node){	
        ...
        struct  sr_rt* tableMatch = longestPrefixMatch(sr,ipSource);
	if (!tableMatch){
		return;
	}
	...
	struct sr_rt* tableMatch  = longestPrefixMatch(sr,iphdr->ip_src);
	if (!tableMatch){
		return;
	}
    ...
    struct sr_rt* tableMatch = longestPrefixMatch(sr,iphdr->ip_dst);
        ...
        char* interface/* lent */)
{
  ...
  if(ethernetType == ethertype_arp){
	sr_handlepacket_arp(sr,packet,len,interfaceEntry);
  }
  else{
	printf("called");
...
void sr_attempt_send(struct sr_instance *sr, uint32_t ip_dest, uint8_t *pkt,unsigned int len, struct sr_if *out_iface){
   ...
   if (entry){
        ...
        memcpy( ((sr_ethernet_hdr_t *)pkt)->ether_dhost, mac_address, ETHER_ADDR_LEN);
       ...
       sr_waitforarp(sr, pkt,len, ip_dest,  out_iface);
   }
}
