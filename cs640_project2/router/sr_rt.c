...
struct sr_rt* longestPrefixMatch(struct sr_instance *sr,uint32_t ipDst){
    struct in_addr routingTarget;
    routingTarget.s_addr = ipDst;
    struct sr_rt* routeTable = sr->routing_table; /*Get routing table*/
    struct in_addr largestMask;
    largestMask.s_addr = 0;
   
    struct sr_rt* match = NULL;
    while(routeTable){
        uint32_t result = routeTable->mask.s_addr & routingTarget.s_addr; /*Find the prefix match shared*/
	if(result == routeTable->dest.s_addr){  /*Cehck to see if result is in routing table*/
		if(ntohl(routeTable->mask.s_addr) >= ntohl(largestMask.s_addr)){ /*Check to see if match is longest*/
		        match = routeTable; /*New match found*/
			largestMask = routeTable->mask; /*Set new largest mask length*/
		}
	}
        routeTable = routeTable->next; /*Go to the next entry in routing table*/
    }
